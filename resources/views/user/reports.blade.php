    ...
    <div class="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
        <div class="p-6 border-b border-slate-200 bg-slate-50/50">
            ...
            <form id="searchForm" method="GET" action="{{ route('petitions.reports') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">Filter by Seat</label>
                    <select name="seat_id" class="w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500/10 text-sm font-medium text-slate-900 py-2.5 transition-all bg-white">
                        <option value="" class="font-medium text-slate-800 bg-white py-1">All Seats</option>
                        @foreach($seats as $seat)
                            <option value="{{ $seat->seat_id }}" {{ request('seat_id') == $seat->seat_id ? 'selected' : '' }} class="font-medium text-slate-800 bg-white py-1">{{ $seat->seat_name }}</option>
                        @endforeach
                    </select>
                </div>
                @endif
